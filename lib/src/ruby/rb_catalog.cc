...
namespace puppet_transaction { namespace ruby {
    ...
    rb_catalog::rb_catalog(VALUE self, value_registry *registry)
    ...
    bool rb_catalog::has_version() const
    {
        ...
        auto version = ruby.rb_funcall(_self, ver_fun, 0);
    ...
    void rb_catalog::each_key(std::function<void(const string&)> cb) const {
        ...
        auto resource_table = ruby.rb_ivar_get(_self, ruby.rb_intern("@resource_table"));
        auto resources = ruby.rb_ivar_get(_self, ruby.rb_intern("@resources"));
    ...
    shared_ptr<resource> rb_catalog::get_resource(const string& key) const {
        auto& ruby = api::instance();
        auto resource_ref = ruby.utf8_value(key);
        auto res_fun = ruby.rb_intern("resource");
        auto the_resource = ruby.rb_funcall(_self, res_fun, 1, resource_ref);
        if (the_resource == ruby.nil_value()) {
            return nullptr;
        } else  {
            return value_wrapper::wrap<rb_resource>(the_resource, _registry);
        }
    }
}}
